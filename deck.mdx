...
upgrading `react` on the web means:
...
som devs had ideas to automate this, and we got the original `react-native upgrade`, `rnpm upgrade`, `react-native-git-upgrade`, the rn-diff repo, rn-diff-purge. suffice it to say there were many iterations.
